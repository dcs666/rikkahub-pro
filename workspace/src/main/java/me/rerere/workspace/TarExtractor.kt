...
/**
 * 纯 Java tar 解压器（支持 gzip/xz、GNU 长名、PAX、symlink/hardlink、权限位）。
 *
 * 从 RootfsInstaller 提取，供 rootfs 安装与 ProotUpdater（deb 包解压）共用。
 * 安全设计：路径穿越防护（.. 拒绝 / 逃逸目录拒绝）、PAX 死循环防护、特殊头处理。
 */
internal object TarExtractor {
    ...
    fun extract(
        ...
        stripPrefix: String = "",
        onProgress: (entries: Int, currentEntry: String) -> Unit = { _, _ -> },
    ) {
        format.wrapStream(BufferedInputStream(archive.inputStream())).use { input ->
            ...
            while (true) {
                ...
                val stripped = stripPrefix(header.name, stripPrefix)
                if (stripped == null) {
                    // 不在 stripPrefix 前缀下（deb 包中非 prefix 的文件），跳过数据区
                    input.skipFully(header.size.paddedTarSize())
                    continue
                }
                val target = targetDir.safeResolve(stripped)
                ...
                entries++
                onProgress(entries, header.name)
            }
        }
    }

    /** 去掉 stripPrefix 前缀；不在前缀下返回 null（跳过该条目） */
    private fun stripPrefix(name: String, stripPrefix: String): String? {
        if (stripPrefix.isEmpty()) return name
        val normalized = stripPrefix.trimStart('/').trimEnd('/')
        val candidate = name.trimStart('/')
        return when {
            candidate == normalized -> ""
            candidate.startsWith("$normalized/") -> candidate.removePrefix("$normalized/")
            else -> null
        }
    }
